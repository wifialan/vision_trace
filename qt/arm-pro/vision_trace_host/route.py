...
class PathRoute:
    def __init__(self):
        with open('/home/igosens/huawei/software/path_node.json', 'r') as file:
...
#                self.cross_exit_coord = n + 1
    ...
    def plan_straight_road(self, start_node, stop_node):
            # 直线规划，设A为当前节点，B为目标节点，共有两种情况
            # 1. A在B前
            # 2. A在B后
            self.path_plan_indicate.clear()
            if start_node < stop_node:
                for i in range(start_node, stop_node):
                    self.path_plan_indicate.append('UP')
            else:
                self.path_plan_indicate.append('TURN')
                for i in range(stop_node, start_node):
                    self.path_plan_indicate.append('UP')
